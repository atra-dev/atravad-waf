...
const isVercelOriginUrl = (originUrl) =>
  getOriginHostname(originUrl).endsWith(".vercel.app");
...
const applyRecommendedOriginDefaults = (data) => {
  ...
  return {
    ...
    originAuthHeaderName: data.originAuthHeaderValue?.trim()
...
const validateSslInput = (data) => {
  ...
  if (!isValidPemCertificate(cert)) {
    return {
      valid: false,
      message:
        "Certificate must be PEM format (BEGIN CERTIFICATE / END CERTIFICATE).",
    };
  }
  if (!isValidPemPrivateKey(key)) {
    return {
      valid: false,
      message:
        "Private key must be PEM format (BEGIN PRIVATE KEY / END PRIVATE KEY).",
    };
  }
...
const mutedPanelClassName = "theme-inset-surface rounded-xl p-4";
...
const modalHelpTextClassName = "mt-1 text-xs theme-text-muted";

const modalTitleClassName = "text-2xl font-bold theme-text-primary";

const modalBodyTextClassName = "theme-text-secondary";
...
export default function AppsPage() {
        ...
        origins: [
          {
            ...
            upstreamHost:
              preparedFormData.originUpstreamHost?.trim() || undefined,
            tlsServername:
              preparedFormData.originTlsServername?.trim() || undefined,
            authHeaderName:
              preparedFormData.originAuthHeaderName?.trim() || undefined,
            authHeaderValue:
              preparedFormData.originAuthHeaderValue?.trim() || undefined,
            websocketEnabled: preparedFormData.websocketEnabled !== false,
            websocketIdleTimeoutSec:
              preparedFormData.websocketEnabled !== false
                ? Number.parseInt(
                    preparedFormData.websocketIdleTimeoutSec,
                    10,
                  ) || 900
                : undefined,
            weight: 100,
            ...
            responseBuffering:
              preparedFormData.responseInspectionEnabled !== false,
          },
        ],
        policyId: preparedFormData.policyId || null,
        responseInspectionEnabled:
          preparedFormData.responseInspectionEnabled !== false,
        ssl:
          preparedFormData.sslMode === SSL_MODE.CUSTOM
            ? {
                customCert: true,
                cert: preparedFormData.customCert?.trim() || "",
                key: preparedFormData.customKey?.trim() || "",
                fullchain: preparedFormData.customFullchain?.trim() || null,
              }
            : {
                autoProvision: preparedFormData.autoProvisionSSL !== false,
                customCert: false,
              },
  ...
  const handleOpenEdit = (app) => {
    ...
    setEditFormData({
      ...
      websocketIdleTimeoutSec: String(
        app.origins?.[0]?.websocketIdleTimeoutSec || 900,
      ),
  ...
  const handleDeleteSite = async (app) => {
    if (
      !confirm(
        `Are you sure you want to delete "${app.domain}"? This action cannot be undone.`,
      )
    ) {
      return;
    }
      ...
      const response = await fetch(`/api/apps/${selectedAppForEdit.id}`, {
        ...
        body: JSON.stringify({
          origins: [
            {
              ...
              upstreamHost:
                preparedEditFormData.originUpstreamHost?.trim() || undefined,
              tlsServername:
                preparedEditFormData.originTlsServername?.trim() || undefined,
              authHeaderName:
                preparedEditFormData.originAuthHeaderName?.trim() || undefined,
              authHeaderValue:
                preparedEditFormData.originAuthHeaderValue?.trim() || undefined,
              websocketEnabled: preparedEditFormData.websocketEnabled !== false,
              websocketIdleTimeoutSec:
                preparedEditFormData.websocketEnabled !== false
                  ? Number.parseInt(
                      preparedEditFormData.websocketIdleTimeoutSec,
                      10,
                    ) || 900
                  : undefined,
              weight: 100,
              healthCheck: { path: "/health", interval: 30, timeout: 5 },
              responseBuffering:
                preparedEditFormData.responseInspectionEnabled !== false,
            },
          ],
          ...
          responseInspectionEnabled:
            preparedEditFormData.responseInspectionEnabled !== false,
          ssl:
            preparedEditFormData.sslMode === SSL_MODE.CUSTOM
              ? {
                  customCert: true,
                  cert: preparedEditFormData.customCert?.trim() || "",
                  key: preparedEditFormData.customKey?.trim() || "",
                  fullchain:
                    preparedEditFormData.customFullchain?.trim() || null,
                }
              : {
                  autoProvision: preparedEditFormData.autoProvisionSSL,
                  customCert: false,
                },
        }),
      });
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="max-w-2xl">
                ...
                <p className="mt-3 text-sm leading-7 text-slate-700 dark:text-slate-300">
                  Website onboarding is handled by the ATRAVA Defense super
                  admin team. Your account must be assigned to a managed tenant
                  before you can add or manage protected sites.
                </p>
                <p className="mt-2 text-sm leading-7 text-slate-600 dark:text-slate-400">
                  Contact the ATRAVA Defense operations team to provision your
                  organization, create user access, and complete managed
                  onboarding.
                </p>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <h1 className="text-2xl font-bold theme-text-primary">
                  All Websites ({filteredApps.length}/{apps.length})
                </h1>
                      ...
                      <div className="px-5 pb-0 pt-5 sm:px-6 sm:pt-6">
                        <div className="flex items-start justify-between gap-3">
                          ...
                          <div className="relative">
                            <button
                              onClick={(e) =>
                                openSettingsWithPosition(e, app.id)
                              }
                        ...
                        <div className="mt-4 space-y-2.5 text-[14px] leading-5.5 sm:text-[15px] sm:leading-6">
                          ...
                          <div className="flex flex-wrap items-center gap-x-2 gap-y-1">
                            ...
                            <span
                              className={`inline-flex items-center rounded-md px-2.5 py-1 text-[11px] font-semibold tracking-wide ${sslMeta.tone}`}
                            >
                              {sslMeta.label}
                            </span>
                          </div>
                        ...
                        {activated && (
                          <div className="mt-5 flex flex-wrap items-center gap-x-1.5 gap-y-1.5 text-[13px] font-medium theme-text-secondary sm:text-[14px]">
                            <Link
                              href="/logs"
                              className="rounded-md px-1 py-0.5 transition hover:text-cyan-700 dark:hover:text-cyan-300"
                            >
                              Reports
                            </Link>
                              ...
                              <div className="flex items-end gap-5">
                                <div className="flex h-16 items-end gap-2">
                                  <div
                                    ...
                                    style={{
                                      height: `${getTrafficBarHeight(stats.blocked, maxTraffic)}px`,
                                    }}
                                  />
                                  <div
                                    className="w-7 rounded-t-sm bg-emerald-200"
                                    style={{
                                      height: `${getTrafficBarHeight(stats.allowed, maxTraffic)}px`,
                                    }}
                                  />
                                </div>
                                <div className="space-y-1 text-sm">
                                  <div className="flex items-end gap-6">
                                    <div className="text-center">
                                      <div className="text-[1.9rem] font-semibold leading-none text-red-500">
                                        {stats.blocked.toLocaleString()}
                                      </div>
                                      <div className="mt-1 text-[13px] theme-text-secondary">
                                        Blocked
                                      </div>
                                    </div>
                                    <div className="text-center">
                                      <div className="text-[1.9rem] font-semibold leading-none text-emerald-500">
                    ...
                    {filteredApps.map((app) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-6 py-4">
                            <span
                              className={`inline-flex items-center px-2 py-0.5 text-xs font-medium rounded ${sslMeta.tone}`}
                            >
                              {sslMeta.label}
                            </span>
                          </td>
                  ...
                  <div className={modalShellClassName}>
                    ...
                    <div className="p-6 overflow-y-auto flex-1 min-h-0">
                      ...
                      {wizardStep === 2 && (
                        <div className="space-y-6">
                          <h2 className={modalTitleClassName}>
                            Where is your origin server?
                          </h2>
                          <p className={modalBodyTextClassName}>
                            Enter the URL of your actual web server. Traffic
                            will be forwarded here after WAF inspection.
                          </p>
                          <input
                            type="url"
                            ...
                            onChange={(e) =>
                              setFormData(
                                applyRecommendedOriginDefaults({
                                  ...formData,
                          ...
                          {isVercelOriginUrl(formData.originUrl) && (
                            <div className="rounded-xl border border-rose-200 bg-rose-50 p-4 text-sm text-rose-900 dark:border-rose-900/50 dark:bg-rose-950/40 dark:text-rose-100">
                              This client origin is on Vercel. ATRAVA Defense
                              will auto-fill the upstream host, TLS server name,
                              and recommended origin auth header name. If you
                              want to prevent direct bypass traffic, configure
                              the client origin to reject requests that do not
                              include the shared header value.
                            </div>
                          )}
                            ...
                            <div>
                              ...
                              <input
                                ...
                                onChange={(e) =>
                                  setFormData({
                                    ...formData,
                                    originUpstreamHost: e.target.value,
                                  })
                                }
                              />
                              <p className={modalHelpTextClassName}>
                                Leave empty to forward the visitor&apos;s
                                original host header. Set this when the origin
                                expects a different host.
                              </p>
                            </div>
                            <div>
                              <label className="mb-2 block text-sm font-medium theme-text-secondary">
                              ...
                              <input
                                ...
                                onChange={(e) =>
                                  setFormData({
                                    ...formData,
                                    originTlsServername: e.target.value,
                                  })
                                }
                              />
                              <p className={modalHelpTextClassName}>
                                Use this when your origin URL is an IP or
                                provider hostname, but the HTTPS certificate
                                expects a different name. Example: connect to
                                `https://1.2.3.4` using `app.example.com` for
                                TLS.
                              </p>
                            </div>
                          </div>
                            ...
                            <div>
                              ...
                              <input
                                ...
                                onChange={(e) =>
                                  setFormData({
                                    ...formData,
                                    originAuthHeaderName: e.target.value,
                                  })
                                }
                              />
                              <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
                                Optional header name ATRAVA Defense will send to
                                your origin. This is especially useful when the
                                client origin is publicly reachable, such as on
                                Vercel.
                              </p>
                            </div>
                            <div>
                              ...
                              <input
                                ...
                                onChange={(e) =>
                                  setFormData({
                                    ...formData,
                                    originAuthHeaderValue: e.target.value,
                                  })
                                }
                              />
                              <p className={modalHelpTextClassName}>
                                The shared secret value your origin verifies.
                                Use this to block direct bypass traffic that
                                does not come through ATRAVA Defense.
                              </p>
                            </div>
                          </div>
                            ...
                            <div className="flex items-center justify-between gap-4">
                              <div>
                                <p className="text-sm font-medium text-amber-900 dark:text-amber-200">
                                  Allow WebSockets through ATRAVA Defense
                                </p>
                                <p className="mt-1 text-xs text-amber-800 dark:text-amber-300/80">
                                  ATRAVA Defense inspects the WebSocket
                                  handshake, then tunnels frames. Post-upgrade
                                  frames are not inspected individually.
                                </p>
                              </div>
                              <input
                                type="checkbox"
                              ...
                              <div className="mt-3">
                                ...
                                <input
                                  ...
                                  onChange={(e) =>
                                    setFormData({
                                      ...formData,
                                      websocketIdleTimeoutSec: e.target.value,
                                    })
                                  }
                                />
                              </div>
                          ...
                          <label className="theme-inset-surface flex items-center justify-between rounded-xl px-4 py-3">
                            <span className="pr-4 text-sm theme-text-secondary">
                              ...
                              <span className="block text-xs theme-text-muted">
                                Disable this for SSE, AI streaming, or
                                serverless responses that must stay streamed.
                              </span>
                            </span>
                            <input
                              type="checkbox"
                              checked={
                                formData.responseInspectionEnabled !== false
                              }
                              onChange={(e) =>
                                setFormData({
                                  ...formData,
                                  responseInspectionEnabled: e.target.checked,
                                })
                              }
                              className="h-4 w-4 text-teal-600 focus:ring-teal-500 rounded"
                            />
                          ...
                          <div className="space-y-4">
                            <label className={modalSectionLabelClassName}>
                              SSL Certificate
                            </label>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                              <label
                                className={`cursor-pointer rounded-2xl border p-4 transition-colors ${formData.sslMode === SSL_MODE.AUTO ? "border-teal-500 bg-teal-50 dark:bg-teal-950/35" : "theme-soft-surface hover:border-teal-300 dark:hover:border-teal-700"}`}
                              >
                                <div className="flex items-start gap-3">
                                  ...
                                  <div>
                                    ...
                                    <p className="mt-1 text-xs theme-text-secondary">
                                      Let&apos;s Encrypt certificate issued and
                              ...
                              <label
                                className={`cursor-pointer rounded-2xl border p-4 transition-colors ${formData.sslMode === SSL_MODE.CUSTOM ? "border-teal-500 bg-teal-50 dark:bg-teal-950/35" : "theme-soft-surface hover:border-teal-300 dark:hover:border-teal-700"}`}
                              >
                                <div className="flex items-start gap-3">
                                  <input
                                    type="radio"
                                  ...
                                  <div>
                                    ...
                                    <p className="mt-1 text-xs theme-text-secondary">
                                      Paste PEM or import certificate/key files.
                                    </p>
                                  </div>
                                </div>
                              </label>
                            ...
                            {formData.sslMode === SSL_MODE.AUTO && (
                              <label className="theme-inset-surface flex items-center justify-between rounded-xl px-4 py-3">
                                <span className="text-sm theme-text-secondary">
                                  Automatically provision and renew SSL
                                  certificate
                                </span>
                                <input
                                  type="checkbox"
                                  ...
                                  onChange={(e) =>
                                    setFormData({
                                      ...formData,
                                      autoProvisionSSL: e.target.checked,
                                    })
                                  }
                                  className="h-4 w-4 text-teal-600 focus:ring-teal-500 rounded"
                                />
                            ...
                            {formData.sslMode === SSL_MODE.CUSTOM && (
                              <div className="mt-3 space-y-3 border-l-2 border-teal-200 pl-4 dark:border-teal-900">
                                ...
                                <div>
                                  ...
                                  <textarea
                                    ...
                                    onChange={(e) => {
                                      ...
                                      setFormData({
                                        ...formData,
                                        customFullchain: e.target.value,
                                      });
                                    }}
                                    rows={2}
                                ...
                                {(createSslUiError ||
                                  formData.customCert ||
                                  formData.customKey ||
                                  formData.customFullchain) && (
                                  <p
                                    className={`text-xs ${createSslValidation.valid ? "text-emerald-700" : "text-red-600"}`}
                                  >
                                    {createSslUiError ||
                                      createSslValidation.message}
                                  </p>
                                )}
                              </div>
                            )}
                      ...
                      {wizardStep === 3 && (
                        <div className="py-12 text-center">
                          ...
                          <p className="mt-6 text-lg theme-text-secondary">
                        ...
                        <div className="space-y-6">
                          <div className="text-center">
                            ...
                            <p className="mt-2 theme-text-secondary">
                              Point your domain&apos;s DNS to activate
                              protection.
                            </p>
                          </div>
                          ...
                          <div className={`${mutedPanelClassName} space-y-3`}>
                            ...
                            <div className="mt-2 rounded-xl border border-blue-200 bg-blue-50 p-3 dark:border-blue-900/50 dark:bg-blue-950/35">
                              <p className="text-sm text-blue-800 dark:text-blue-200">
                                <strong>Next Step:</strong> After adding your
                                site, you&apos;ll receive a Firewall IP address
                                based on your origin server&apos;s location.
                  ...
                  <div className={modalShellClassName}>
                    ...
                    >
                      <div className="p-6 space-y-5 overflow-y-auto flex-1 min-h-0">
                        ...
                        <div
                          className={
                            editModalTab !== "general" ? "hidden" : "space-y-5"
                          }
                        >
                          <div>
                            <label className={modalSectionLabelClassName}>
                          ...
                          <div>
                            ...
                            {isVercelOriginUrl(editFormData.originUrl) && (
                              <div className="mt-3 rounded-xl border border-rose-200 bg-rose-50 p-4 text-sm text-rose-900 dark:border-rose-900/50 dark:bg-rose-950/40 dark:text-rose-100">
                                This client origin is on Vercel. Keep the
                                upstream host and TLS server name aligned to the
                                `vercel.app` hostname. If you want to block
                                direct traffic that bypasses ATRAVA Defense,
                                enforce the shared origin header in the client
                                origin app.
                              </div>
                            )}
                          </div>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                              ...
                              />
                              <p className={modalHelpTextClassName}>
                                Leave empty to pass the visitor&apos;s original
                                host header to the origin.
                              </p>
                            </div>
                            ...
                            <div>
                              ...
                              />
                              <p className={modalHelpTextClassName}>
                                Set this when the origin URL is an IP or
                                provider host, but HTTPS upstream must present a
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                              ...
                              />
                              <p className={modalHelpTextClassName}>
                                Optional header name ATRAVA Defense sends to the
                                origin. This is especially useful when the
                                client origin is publicly reachable, such as on
                                Vercel.
                              </p>
                            </div>
                          ...
                          <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 dark:border-amber-900/50 dark:bg-amber-950/40">
                            <div className="flex items-center justify-between gap-4">
                              <div>
                                ...
                                <p className="mt-1 text-xs text-amber-800 dark:text-amber-300/80">
                                  The handshake is inspected by the WAF. After
                                  the upgrade, frames pass through as a tunnel.
                                </p>
                              </div>
                              <input
                                type="checkbox"
                          ...
                          <label className="theme-inset-surface flex items-center justify-between rounded-xl px-4 py-3">
                            <span className="pr-4 text-sm theme-text-secondary">
                              Inspect and buffer origin responses
                              <span className="block text-xs theme-text-muted">
                                Disable this for streaming SSR, SSE, AI
                                responses, and similar serverless output.
                              </span>
                            </span>
                            <input
                              type="checkbox"
                              checked={
                                editFormData.responseInspectionEnabled !== false
                              }
                              onChange={(e) =>
                                setEditFormData({
                                  ...editFormData,
                        ...
                        <div
                          className={
                            editModalTab !== "ssl" ? "hidden" : undefined
                          }
                        >
                          <div className="space-y-4">
                            <p className="text-sm theme-text-secondary">
                              Choose how SSL/TLS is provided for this site.
                            </p>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                              <label
                                className={`cursor-pointer rounded-2xl border p-4 transition-colors ${editFormData.sslMode === SSL_MODE.AUTO ? "border-teal-500 bg-teal-50 dark:bg-teal-950/35" : "theme-soft-surface hover:border-teal-300 dark:hover:border-teal-700"}`}
                              >
                                <div className="flex items-start gap-3">
                                  <input
                                    type="radio"
                              ...
                              <label
                                className={`cursor-pointer rounded-2xl border p-4 transition-colors ${editFormData.sslMode === SSL_MODE.CUSTOM ? "border-teal-500 bg-teal-50 dark:bg-teal-950/35" : "theme-soft-surface hover:border-teal-300 dark:hover:border-teal-700"}`}
                              >
                                <div className="flex items-start gap-3">
                                  <input
                                    type="radio"
                                  ...
                                  <div>
                                    ...
                                    <p className="mt-1 text-xs theme-text-secondary">
                                      Upload or paste your own PEM certificate
                                      set.
                                    </p>
                                  </div>
                                </div>
                            ...
                            {editFormData.sslMode === SSL_MODE.AUTO && (
                              <label className="theme-inset-surface flex items-center justify-between rounded-xl px-4 py-3">
                                ...
                                <input
                                  type="checkbox"
                                  checked={
                                    editFormData.autoProvisionSSL !== false
                                  }
                                  onChange={(e) =>
                                    setEditFormData({
                                      ...editFormData,
                            ...
                            {editFormData.sslMode === SSL_MODE.CUSTOM && (
                              <div className="mt-4 space-y-3 border-t border-[var(--border-soft)] pt-4">
                                ...
                                <div>
                                  ...
                                  <textarea
                                    placeholder={
                                      selectedAppForEdit?.ssl
                                        ?.hasStoredCustomCert
                                        ? "Leave blank to keep the existing certificate, or paste a replacement."
                                        : "-----BEGIN CERTIFICATE-----..."
                                    }
                                ...
                                <div>
                                  ...
                                  <textarea
                                    placeholder={
                                      selectedAppForEdit?.ssl
                                        ?.hasStoredCustomCert
                                        ? "Leave blank to keep the existing private key, or paste a replacement."
                                        : "-----BEGIN PRIVATE KEY-----..."
                                    }
                                ...
                                <div>
                                  ...
                                  <textarea
                                    placeholder={
                                      selectedAppForEdit?.ssl
                                        ?.hasStoredCustomCert
                                        ? "Leave blank to keep the existing chain, or paste a replacement."
                                        : "Optional intermediate + root certificates"
                                    }
                                ...
                                {(editSslUiError ||
                                  editFormData.customCert ||
                                  editFormData.customKey ||
                                  editFormData.customFullchain) && (
                                  <p
                                    className={`text-xs ${editSslValidation.valid ? "text-emerald-700" : "text-red-600"}`}
                                  >
                                    {editSslUiError ||
                                      editSslValidation.message}
                                  </p>
                                )}
                              </div>
                            )}
                          ...
                          <>
                            <div className="text-center">
                              ...
                              <p className="mt-2 theme-text-secondary">
                                {setupActivated ? (
                                  <>
                                    Your domain is pointing to ATRAVA Defense
                                    and protection is active for{" "}
                                    <strong>
                                      {selectedAppForSetup.domain}
                                    </strong>
                                  </>
                                ) : (
                                  <>
                                    Point your domain&apos;s DNS to activate WAF
                                    protection for{" "}
                                    <strong>
                                      {selectedAppForSetup.domain}
                                    </strong>
                            ...
                            <div className="theme-inset-surface rounded-2xl p-5">
                              ...
                              <div className="space-y-3">
                                <div className="theme-soft-surface flex items-center justify-between rounded-xl p-3">
                                  <div>
                                    <span className="text-xs font-medium uppercase theme-text-muted">
                                      Domain
                                    </span>
                                    <p className="font-mono text-sm font-medium theme-text-primary">
                                      {selectedAppForSetup.domain}
                                    </p>
                                  </div>
                                </div>
                                ...
                                <div className="theme-soft-surface flex items-center justify-between rounded-xl p-3">
                                  <div>
                                    ...
                                    <p className="font-mono text-sm theme-text-secondary">
                                      {getHostingDisplay(selectedAppForSetup)}
                                    </p>
                                  </div>
                                </div>
                            ...
                            >
                              ...
                              >
                                ...
                                <li>
                                  Find DNS settings for{" "}
                                  <strong>{selectedAppForSetup.domain}</strong>
                                </li>
                                <li>
                                  {setupActivated ? (
                                    <>

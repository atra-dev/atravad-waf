...
function getUsageTone(percentUsed) {
  if (percentUsed >= 100) return 'text-red-700 bg-red-50/90 border-red-200';
  if (percentUsed >= 80) return 'text-amber-700 bg-amber-50/90 border-amber-200';
  return 'text-emerald-700 bg-emerald-50/90 border-emerald-200';
}
...
export default function TenantPlanBanner({ tenant, resources = [] }) {
  ...
  return (
    <div className="relative overflow-hidden rounded-[30px] border border-slate-200/80 bg-[linear-gradient(135deg,#fbfdff_0%,#f4f8ff_48%,#f8fcfb_100%)] p-6 shadow-[0_18px_60px_rgba(15,23,42,0.08)]">
      ...
      <div className="relative flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
        <div className="max-w-2xl">
          ...
          <h3 className="mt-3 text-2xl font-semibold tracking-tight text-slate-950 sm:text-[2rem]">
      ...
      {entries.length > 0 ? (
        <div className="relative mt-6 grid gap-3 md:grid-cols-3">
          {entries.map((entry) => (
            <div
              key={entry.label}
              className="rounded-2xl border border-white/70 bg-white/80 px-4 py-4 backdrop-blur-sm"
            >

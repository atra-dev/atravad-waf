# ATRAVAD WAF Architecture & Connection Diagram
...
## 2. ATRAVAD WAF Traffic Flow (User → WAF → Origin)
...
sequenceDiagram
    ...
    DNS->>User: Resolves to WAF IP (or CNAME → WAF IP)
...
- **Atravad WAF traffic** is: **User → DNS (WAF IP) → WAF Proxy (AWS or Data Center) → ModSecurity → Origin** (or block at WAF).
...
![ATRAVAD WAF Architecture](atravad-waf-architecture.svg)
